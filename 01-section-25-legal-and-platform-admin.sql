...
-- Table-level SELECT grant so the RLS policy above is reachable by signed-in
-- users (RLS narrows an existing grant; without the grant the policy grants
-- nothing). No write grants — agreements are managed out-of-band.
