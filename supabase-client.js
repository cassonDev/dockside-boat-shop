...
// ONE shop. This is the tenant-scoped staff status — it does NOT touch
// profiles.active (the account-level flag). RLS ("memberships: owner manage")
// permits this only for an owner of that shop, so passing shopId is safe: an
// owner can never flip a membership in a shop they don't own.
export async function setMembershipActive({ profileId, shopId, active }) {
  const { data, error } = await supabase.from('shop_memberships')
    .update({ is_active: active })
    .eq('profile_id', profileId).eq('shop_id', shopId)
    .select('*').single();
  if (error) throw error;
